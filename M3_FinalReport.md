...
**Scenerio 1-- Crowd Strike** This scenerio is designed to simulate the network vulnerabilities associated with the CrowdStrike outage. It shows the effects of a potential cyberattack by illustrating how a network can be brought down, the extent of the disruption, and the timeline required for the recovery of services. The goal is to demonstrate network outage and provide insight into the time it takes to bring the network back online.  
In Step 1 click the Setup and go buttons to begin creating the netowork. The optimal number of nodes to create a visually clear model is 200 to 400 nodes. Make sure to click the go button and redo layout button to stop the increase of nodes and layout movement.  
In Step 2 bad-update-percent slider should be left at 70%. This shows that not all connections are part of the crowdstrike program and that not all customers received the update. Click the inital update push and Subsequent Update spread. Click the Subsequent Update spread button to stop.  
In Step 3 set the update-fix-chance slider to 1.7 and the update-delay to 4. Then click the update-fix-setup button and the update-fix-go. The Node Status Over Time graph will begin to collect data. You can click the update-fix-go button once all red nodes have turned green.  
