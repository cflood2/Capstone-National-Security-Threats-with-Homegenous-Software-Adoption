...
**Crowd Strike Scenerio** This scenerio is designed to simulate the network vulnerabilities associated with the CrowdStrike outage. It shows the effects of a potential cyberattack by illustrating how a network can be brought down, the extent of the disruption. The goal is to demonstrate network outage and provide insight into the time it takes to bring the network back online.   
In Step 2 bad-update-percent slider should be left at 70%. This shows that not all connections are part of the crowdstrike program and that not all customers received the update.   
In Step 3 set the update-fix-chance slider to 1.7 and the update-delay to 4. You can click the update-fix-go button once all red nodes have turned green.  
